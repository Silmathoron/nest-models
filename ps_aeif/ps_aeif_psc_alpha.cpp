/*
 *  ps_aeif_psc_alpha.cpp
 *
 */

#include "ps_aeif_psc_alpha.h"
...
nest::RecordablesMap< mynest::ps_aeif_psc_alpha > mynest::ps_aeif_psc_alpha::recordablesMap_;
...
namespace nest // template specialization must be placed in namespace
{
  ...
  template <>
  void
  RecordablesMap< mynest::ps_aeif_psc_alpha >::create()
  {
    insert_( names::V_m, &mynest::ps_aeif_psc_alpha::get_y_elem_< mynest::ps_aeif_psc_alpha::State_::V_M > );
    insert_( names::I_ex, &mynest::ps_aeif_psc_alpha::get_y_elem_< mynest::ps_aeif_psc_alpha::State_::I_EXC > );
    insert_( names::I_in, &mynest::ps_aeif_psc_alpha::get_y_elem_< mynest::ps_aeif_psc_alpha::State_::I_INH > );
    insert_( names::w, &mynest::ps_aeif_psc_alpha::get_y_elem_< mynest::ps_aeif_psc_alpha::State_::W > );
  }
}
...
extern "C" int
mynest::ps_aeif_psc_alpha_dynamics( double, const double y[], double f[], void* pnode )
{
  // a shorthand
  typedef mynest::ps_aeif_psc_alpha::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const mynest::ps_aeif_psc_alpha& node = *( reinterpret_cast< mynest::ps_aeif_psc_alpha* >( pnode ) );
...
mynest::ps_aeif_psc_alpha::Parameters_::Parameters_()
...
mynest::ps_aeif_psc_alpha::State_::State_( const Parameters_& p )
  : r_( 0 )
  , r_offset_ ( 0. )
{
  y_[ 0 ] = p.E_L;
...
mynest::ps_aeif_psc_alpha::State_::State_( const State_& s )
  : r_( s.r_ )
...
mynest::ps_aeif_psc_alpha::State_& mynest::ps_aeif_psc_alpha::State_::operator=( const State_& s )
{
...
void
mynest::ps_aeif_psc_alpha::Parameters_::get( DictionaryDatum& d ) const
{
...
void
mynest::ps_aeif_psc_alpha::Parameters_::set( const DictionaryDatum& d )
{
...
void
mynest::ps_aeif_psc_alpha::State_::get( DictionaryDatum& d ) const
{
...
void
mynest::ps_aeif_psc_alpha::State_::set( const DictionaryDatum& d, const Parameters_& )
{
...
mynest::ps_aeif_psc_alpha::Buffers_::Buffers_( ps_aeif_psc_alpha& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
mynest::ps_aeif_psc_alpha::Buffers_::Buffers_( const Buffers_&, ps_aeif_psc_alpha& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
mynest::ps_aeif_psc_alpha::ps_aeif_psc_alpha()
  : Archiving_Node()
  , P_()
  , S_( P_ )
  , B_( *this )
...
mynest::ps_aeif_psc_alpha::ps_aeif_psc_alpha( const ps_aeif_psc_alpha& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
mynest::ps_aeif_psc_alpha::init_state_( const Node& proto )
{
  const ps_aeif_psc_alpha& pr = downcast< ps_aeif_psc_alpha >( proto );
  S_ = pr.S_;
}

void
mynest::ps_aeif_psc_alpha::init_buffers_()
{
  B_.events_.resize();
  B_.events_.clear();
  ...
  B_.sys_.function = ps_aeif_psc_alpha_dynamics;
...
void
mynest::ps_aeif_psc_alpha::calibrate()
{
...
void
mynest::ps_aeif_psc_alpha::interpolate_( double& t, double t_old )
{
...
void
mynest::ps_aeif_psc_alpha::spiking_( const long_t T, const long_t lag, const double t )
{
  // spike event
  SpikeEvent se;
...
void
mynest::ps_aeif_psc_alpha::update( const Time& origin, const nest::long_t from, const nest::long_t to )
{
  ...
  */
  ...
  for ( long_t lag = from; lag < to; ++lag )
  {
    ...
    while ( t < B_.step_ )
    {
      // store the previous values of V_m, I_exc, I_inh, and t
...
void
mynest::ps_aeif_psc_alpha::handle( SpikeEvent& e )
{
...
void
mynest::ps_aeif_psc_alpha::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
mynest::ps_aeif_psc_alpha::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
